...
def run_script():
    #Soft Setting
    clean_language = combobox_lang.get()
    clean_number = combobox_lang.get()
    clean_date = entry_date.get()
    clean_target = entry_target.get()
    clean_start_piece = int(entry_start.get())
    clean_maximum_word_len = int(entry_max_word.get())
    clean_maximum_query_len = int(entry_max_query.get())

    #Hard setting
    target_language_dict = {'fr':['fr'],
                            'en':['en'],
                            'sw':['sw'],
                            'ng':['Nigerian Pidgin(pcm)'],
...
if __name__ == '__main__':
        #Build Window
    container = tk.Tk()
    container.title('Identify') #Set Title
    ...
    lable_lang = tk.Label(container, text='Identify Lang')
    lable_lang.grid(row=1, column=0, sticky=W)
    divide_lang_turple = ('en', 'ha', 'fr', 'sw')
    ...
    label_target = tk.Label(container, text='Identify Target')
    ...
    label_start = tk.Label(container, text='Identify Start')
    label_start.grid(row=3, column=0, sticky=W)
    default_entry_start = tk.StringVar(value='0')
    entry_start = tk.Entry(container, textvariable=default_entry_start)
    entry_start.grid(row=3, column=1) #Set Start

    label_max_query = tk.Label(container, text='Max Query')
    label_max_query.grid(row=4, column=0, sticky=W)
    default_entry_max_query = tk.StringVar(value='500')
    entry_max_query = tk.Entry(container, textvariable=default_entry_max_query)
    entry_max_query.grid(row=4, column=1) #Set Max Length for Query

    label_maxn_word = tk.Label(container, text='Max Word')
    label_maxn_word.grid(row=5, column=0, sticky=W)
    default_entry_max_word = tk.StringVar(value='15')
    entry_max_word = tk.Entry(container, textvariable=default_entry_max_word)
    entry_max_word.grid(row=5, column=1) #Set Max Length for Word

    #Run Container
    button_run_script = tk.Button(container, text='Run', command=run_script)
    button_run_script.grid(row=6, column=0) #Run Script Button

    button_quit_script = tk.Button(container, text='Quit', command=container.quit)
    button_quit_script.grid(row=6, column=1) #Quit Script Button
